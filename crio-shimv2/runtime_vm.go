...
func (r *runtimeVM) CreateContainer(c *oci.Container, cgroupParent string) (err error) {
	...
	case <-time.After(oci.ContainerCreateTimeout):
		if err := r.remove(r.ctx, c.ID(), ""); err != nil {
			return err
		}
		<-createdCh
		return errors.Errorf("CreateContainer timeout (%v)", oci.ContainerCreateTimeout)
	}
...
func (r *runtimeVM) StopContainer(ctx context.Context, c *oci.Container, timeout int64) error {
	...
	if err := r.waitCtrTerminate(sig, stopCh, oci.KillContainerTimeout); err != nil {
		logrus.Errorf("%v", err)
		return err
	}
...
func (r *runtimeVM) UpdateContainerStatus(c *oci.Container) error {
	...
	state := c.State()
	state.Status = status
	state.Finished = response.ExitedAt
	state.ExitCode = int32(response.ExitStatus)
...
func (r *runtimeVM) ContainerStats(c *oci.Container) (*oci.ContainerStats, error) {
	...
	return oci.MetricsToCtrStats(c, metrics), nil
}
